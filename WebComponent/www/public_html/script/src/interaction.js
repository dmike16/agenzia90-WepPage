(function() {
  "use strict";
  var tmp = new UtilityBuild(["dmUtil", "dmPaper"], function(obj) {
    /*
    ...
    var scrollingArea = {
      ...
      update: function() {
        if (this.parallax.state) {
          ...
          if (lscroll > 0) {
            ...
            if (!this._allActive) {
              var lim1 = this.px.pages[0],
                lim2 = this.px.pages[1];
      ...
      activeElementWhenScrolledToLim: function(stringEle, lim, callback) {
        if (this.lastScroll > lim) {
          ...
          callback(eleDom);
          var status = this.$._cacheStatusEle,
            p = "";
    ...
    var resizeArea = {
      ...
      mobileResize: function mobileResize() {
        ...
        if (width > 781) {
          ...
          //Recalculate the slideContainer Size
          var rect = slide.$.slideControl.getBoundingClientRect();
          slide._ctx.width = rect.width;
          slide._ctx.height = rect.height;
          slide._ctx.slope = rect.width / 2;
        }
      },
      init: function() {
        this._update = update_.bind(this);
    ...
    SlideControl.prototype = {
      ...
      active: function active() {
        ...
        //Use local variable to speed up the read process
        var link = this.$.a;
        var slideControl = this.$.slideControl;

        // Set the Initial Current position to zerpo
        this._currentPos = 0;

        //Bind this to the Event Gesture Fuction
        ...
        // Calculate the slideControl Dimension
        var rect = slideControl.getBoundingClientRect();
        this._ctx = {
          width: rect.width,
          height: rect.height,
          slope: rect.width / 2
        ...
        // Add the click event on the pagination link
        for (var i = link.length - 1; i >= 0; i--) {
          aEventListener(link[i], 'click', this._click);
        }
        // Add the start gesture control on mouseup or touchstart event
        aEventListener(slideControl, this.events.start, this._gestureStart);

        //Check if the devices has touch support
        //if yes add the event touch(move|end|cancel) on slideControl container
        if (this.events.touchSupport || this.events.pointerEventSupport) {
      ...
      deactive: function deactive() {
        ...
        // Remove Event listener becouse the slide containier it has benn disabled
        for (var i = link.length - 1; i >= 0; i--) {
      ...
      click: function click(e) {
        // Prevent from default link action
        e.preventDefault();

        //Store the event target to read the data-item attribute
        var clicked = e.target;
        var dataItem = parseInt(clicked.getAttribute('data-item'));

        //Find the current item active and the current item fit
        var active = this._id['active'];
        var fit = this._id['fit'];

        //If the active item it's clicked do nothing
        if (dataItem === active) {
          return;
        } else {
          // Change the catalog panel
          this.changeState(active, fit, dataItem - active);
        }
      },
      gestureStart: function gestureStart(e) {
        //On first call decide which version it's used. Thanks to memorize pattern
        var gestureStart;
        if (this.events.pointerEventSupport) {
          gestureStart = function gestureStart(e) {
            e.preventDefault();
            //If more than one touch return
            if (e.targetTouches && e.targetTouches.length > 1) {
              return;
            }
            this.$.slideControl.setPointerCapture(e.pointerId);
            //Calculate the initial touch position
            this._initialPos = this.getPointFromEvent(e);

            //Bind to this the function passed as callback to animateFrame
            if (!this._onMovingAnim) {
              this._onMovingAnim = this.onMovingAnim.bind(this);
            }
          };
        } else if (this.events.touchSupport) {
          gestureStart = function gestureStart(e) {
            e.preventDefault();
            //If more than one touch return
            if (e.targetTouches && e.targetTouches.length > 1) {
          ...
          gestureStart = function gestureStart(e) {
            ...
            //In case of mouse event add at this point the mouse(move|up|out) event
            var slideControl = this.$.slideControl;
      ...
      gestureEnd: function gestureEnd(e) {
        ...
        if (this.events.pointerEventSupport) {
          gestureEnd = function gestureEnd(e) {
            ...
            this.$.slideControl.releasePointerCapture(e.pointerId);
            this.updatePosition();
      ...
      updatePosition: function updatePosition() {
        // Nothing to update
        // the point of touch it has not changed
        if (!this._lastPos) {
          return;
        }
        ...
        var direction = (step === 0) ? step : step / Math.abs(step);

        if (Math.abs(step) > this._ctx.slope) {
          this.changeState(active, fit, direction);
        } else {
          var matriX = 'matrix(1,0,0,1,0,0)';
          _apply(this.$.slideControl, 'transform', matriX, ['webkit']);
        }
      ...
      changeState: function changeState(active, fit, direction) {
        var items = this.items;
        var pagination = this.pagination;

        obj.animatedFrame.cancel(this.frame.id);
        this.frame.step.a = 980 * (direction / Math.abs(direction));
    ...
    dropdownToggle = null;
    obj.PaperMaker('button', {
      element: cssSelectorAll('.paper-fab')[0]
    });
